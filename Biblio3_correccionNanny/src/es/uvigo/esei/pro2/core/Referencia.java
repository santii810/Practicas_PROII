...
abstract public class Referencia {
    ...
    public Referencia(Element e) throws ParsingException {
        Element eltoAutores = e.getFirstChildElement(AUTORES_TAG);
        if (eltoAutores == null){
            throw new ParsingException ("Falta autores");
        }
        this.autores = eltoAutores.getValue(); 

        Element eltoTitulo = e.getFirstChildElement(TITULO_TAG);
        if (eltoTitulo == null){
            throw new ParsingException ("Falta titulo");
        }
        this.titulo = eltoTitulo.getValue();           

        Element eltoAnho = e.getFirstChildElement(ANO_TAG);
        if (eltoAnho == null){
            throw new ParsingException ("Falta año");
        }
        this.ano = Integer.parseInt(eltoAnho.getValue());
    }
